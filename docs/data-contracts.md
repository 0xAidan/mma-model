...
| **Value eligibility** | Bout `eligible_for_value` / `match_value_gate` = matched + nonterminal only (does **not** mean every quote is usable). Quote-level resolver requires alias-visible row + bout nonterminal + selection not locked + market not UNKNOWN + quote AVAILABLE + quote’s own freshness (`source_updated_at` when present else `observed_at`). One fresh book/market never un-stales another. |
| **Availability** | `odds_availability_observations` UNKNOWN blocks only that bookmaker/region/market under the effective alias at cutoff; never infers SUSPENDED/LOCKED; never marks the whole event ACTIVE as a substitute. |
