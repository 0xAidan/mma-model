...
| **Quota** | Cost from The Odds API contract (`current`=1×markets×regions; `historical`=10×markets×regions; fixed endpoints need no market/region counts) plus persisted remaining provenance. Remaining is **account-wide** (the same API key used elsewhere can invalidate local spend plans). Remaining must be fresh within `remaining_max_age_sec` (≤ bootstrap `min_interval_sec`, currently 300s) and the UTC calendar month; otherwise unknown/fail-closed. Missing/stale remaining bootstraps via zero-cost `events` (or defers). Overrides must be `0..monthly_limit`. Each batch is re-evaluated immediately before execution against newest remaining (never N pre-authorized spends of one balance). Backfill/ordinary live preserve `run_reserve`; final-hour live may spend reserve but never exceed actual remaining. Distinguish `deferred` vs `exhausted`. Unknown actual cost stays `None` with provenance (never invent `0`). Sport-wide `batch_key` success is durable; late event members reuse stored quote/availability IDs without re-spend. Card cells are cost-neutral collection coverage with explicit `quote_snapshot_at`/`requested_cutoff` vs `match_reconciliation_as_of` clocks (retrospective reconciliation is not PIT-at-checkpoint). |
