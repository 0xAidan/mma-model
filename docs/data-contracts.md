...
| **Quota** | Cost from The Odds API contract (`current`=1×markets×regions; `historical`=10×markets×regions; fixed endpoints need no market/region counts) plus persisted remaining provenance. Remaining must be fresh within `remaining_max_age_sec` and the UTC calendar month; otherwise unknown. Missing/stale remaining bootstraps via zero-cost `events` (or defers). Overrides must be `0..monthly_limit`. Batch budgets are cumulative within a run (never N independent authorizations of one balance). Backfill/ordinary live preserve `run_reserve`; final-hour live may spend reserve but never exceed actual remaining. Distinguish `deferred` vs `exhausted`. Unknown actual cost stays `None` with provenance (never invent `0`). Batch costs are counted once; card cells are cost-neutral collection coverage (not value-ready). |
