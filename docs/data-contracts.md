...
| **Value eligibility** | Bout `eligible_for_value` / `match_value_gate` = matched + nonterminal only (does **not** mean every quote is usable). Quote-level resolver binds bout identity from the **effective alias at cutoff** (`alias.bout_id`); caller bout mismatch → `alias_bout_mismatch`. Also requires alias-visible row + bout nonterminal + selection not locked + market not UNKNOWN + quote AVAILABLE + quote’s own freshness. One fresh book/market never un-stales another. |
| **Availability** | `odds_availability_observations` UNKNOWN blocks only that bookmaker/region/market under the effective alias at cutoff when UNKNOWN is ≥ the quote’s observed availability evidence; a later available quote clears prior UNKNOWN (`availability_note=recovered_by_newer_quote`, non-blocking). Never infers SUSPENDED/LOCKED. |
