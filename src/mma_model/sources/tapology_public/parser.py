"""Parse public Tapology fighter HTML into typed dicts (DWCS-105)."""
...
SOURCE_TAPOLOGY_PUBLIC = "tapology_public"
REQUIRED_SCHEMA = "tapology_fight_results_v1"
REQUIRED_HEADERS = (
    "result",
    "opponent",
    "event",
...
def parse_fighter_page(html: str) -> dict[str, Any]:
    ...
    fighter_name = str(profile.get("data-name") or "").strip()
    ...
    current = soup.select_one(".currentRecord, [data-record]")
    current_text = None
    if current is not None:
        current_text = str(current.get("data-record") or current.get_text(strip=True) or "") or None
    explicit = soup.select_one("[data-pre-fight-record]")
    explicit_record = None
    if explicit is not None:
        explicit_record = {
            "as_of": explicit.get("data-as-of"),
            **(_parse_record(str(explicit.get("data-pre-fight-record") or "")) or {}),
            "classification": explicit.get("data-classification") or "unknown",
        }
    ...
    return {
        ...
        "explicit_pre_fight_record": explicit_record,
        "bouts": bouts,
        "next_url": next_url,
        "left_truncated": left_truncated,
