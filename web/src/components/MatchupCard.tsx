import type { MatchupRow } from "../generated/dashboard";
import {
  formatAmerican,
  formatDecimal,
  formatEv,
    ...
    >
      ...
      <div className="mt-3 flex flex-wrap gap-2 text-xs">
        <span className="rounded border border-ink-300 bg-ink-50 px-2 py-0.5 font-medium">
          Data strength: {maturityLabel(matchup.maturity)}
        </span>
        <span className="rounded border border-ink-300 bg-ink-50 px-2 py-0.5 font-medium">
      ...
      {(availability === "stale" || freshness === "stale") && (
        <p className="mt-3 rounded border border-amber-700 bg-amber-50 px-3 py-2 text-sm text-amber-950" role="status">
          <span aria-hidden="true">◷ </span>
      ...
      {availability === "unavailable" && (
        <p className="mt-3 rounded border border-slate-600 bg-slate-100 px-3 py-2 text-sm text-slate-900" role="status">
          <span aria-hidden="true">○ </span>
      ...
      <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-3">
        ...
        {hasPublishedEv && prices.exact_ev != null ? (
          <div data-testid="exact-ev-row">
            ...
            <dd className="font-semibold">{formatEv(prices.exact_ev)}</dd>
          </div>
        ) : null}
      </dl>
